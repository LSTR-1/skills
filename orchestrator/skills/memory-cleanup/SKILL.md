...
description: >
  ...
  under ~/.orchestrator/projects/*/memory/. Run after major refactors, every 5+ sessions, or when memory
...
> exists. Affected modules in this skill: `scripts/lib/auto-dream.mjs`, `scripts/lib/config.mjs`, `scripts/lib/memory-cleanup/worktree-sweep.mjs`, `scripts/lib/session-discovery.mjs`, `scripts/lib/session-id.mjs`.
...
The memory system lives at `~/.orchestrator/projects/<encoded-cwd>/memory/` and consists of:
...
1. List all files in the memory directory:
   ```bash
   ls -la ~/.orchestrator/projects/*/memory/ 2>/dev/null | grep "$(basename "$(pwd)")"
   ```
...
- PSA-003: `.trae/rules/parallel-sessions.md`
