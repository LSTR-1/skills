...
1. **Full Gate checks**: TypeScript (0 errors), tests (must pass), lint (must pass, warnings OK)
   Alternatively, run `node "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/run-quality-gate.mjs" --variant full-gate --config "$CONFIG"` for deterministic quality gate execution with structured JSON output.
