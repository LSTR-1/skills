...
4. **Noise reduction filters** (apply before reporting):
   ...
   - The `.orchestrator/`, `.orchestrator/`, and `.orchestrator/` state directories are excluded — they are session artifacts, not code
...
Task({
  ...
  query: "Run discovery probes in embedded mode. Scope: session probes + discovery-probes config. Return findings and stats as a JSON object in a markdown code fence. Do NOT run Phase 4 (triage) or Phase 5 (issue creation) — return after Phase 3.",
