...
> **Platform Note:** State files use the platform's native directory: `.orchestrator/` (Claude Code), `.orchestrator/` (Codex CLI), or `.orchestrator/` (Cursor IDE). Shared metrics live in `.orchestrator/metrics/` (v2) with fallback to `<state-dir>/metrics/` for pre-v2.0 legacy data. See `skills/_shared/platform-tools.md`.
...
> exists. Affected modules in this skill: `scripts/lib/evolve/autopilot-effectiveness.mjs`, `scripts/lib/hardware-pattern-detector.mjs`, `scripts/lib/peer-cards/merger.mjs`, `scripts/lib/peer-cards/reader.mjs`, `scripts/lib/peer-cards/writer.mjs`.
...
1. Read `.orchestrator/metrics/sessions.jsonl` (session history). If it does not exist, check `<state-dir>/metrics/sessions.jsonl` as a legacy fallback (where `<state-dir>` is `.orchestrator/`, `.orchestrator/`, or `.orchestrator/` per platform). If neither exists, warn: "No session history found. Run at least one session first."
...
- Read `.orchestrator/metrics/events.jsonl` (session + wave events) and the registry `sweep.log` at `~/.config/orchestrator/sessions/sweep.log`. Both are optional — missing files produce no candidates.
...
const dispatchAgent = async ({ model, prompt, maxTokens }) => {
  // Coordinator uses Agent tool with subagent_type: "general_purpose_task"
