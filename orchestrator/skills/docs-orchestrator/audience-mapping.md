...
| **session-memory** | `~/.orchestrator/projects/<project>/memory/session-*.md`, `.orchestrator/` outputs | Vault, User | Primary for Vault narratives (status updates, decisions made in conversation). Also primary for User tasks designed interactively. |
...
3. **session-memory** — Session transcript, wave outputs, agent summaries, and test
   results stored in `~/.orchestrator/projects/<project>/memory/session-*.md` and under
...
- session-memory: wave summaries and agent outputs from
  ~/.orchestrator/projects/<project>/memory/session-*.md and .orchestrator/ (provided below)
