...
> exists. Affected modules in this skill: `scripts/lib/config-schema.mjs`, `scripts/lib/config.mjs`.
...
3. **session-memory** — Session transcript, wave outputs, and agent summaries stored
   at `~/.orchestrator/projects/<project>/memory/session-*.md` and under `.orchestrator/`
