...
| `.trae/rules/` has path-scoped rules | `ls .trae/rules/*.md 2>/dev/null` |
| `.orchestrator/settings.json` exists | `ls .orchestrator/settings.json` |
...
- ✗ `.orchestrator/settings.json` missing — create with permissions and hooks
