...
> exists. Affected modules in this skill: `scripts/lib/agent-status.mjs`, `scripts/lib/autopilot.mjs`, `scripts/lib/frontmatter-guard.mjs`, `scripts/lib/quality-gate.mjs`, `scripts/lib/state-md.mjs`, `scripts/lib/wave-executor/pool.mjs`, `scripts/lib/worktree.mjs`.
...
8. **Verification before completion**: Before claiming any task done, run the verification command and quote the evidence inline. See `.trae/rules/verification-before-completion.md`.
...
The fixer-agent prompt MUST include a reminder of `.trae/rules/testing.md` § "Test Quality — False-Positive Prevention"
...
- **NEVER** run fire-and-forget dispatch (NOT supported) during waves — you lose coordination ability
