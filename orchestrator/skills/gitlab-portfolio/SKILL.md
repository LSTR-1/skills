...
> exists. Affected modules in this skill: `scripts/lib/gitlab-portfolio/aggregator.mjs`, `scripts/lib/gitlab-portfolio/markdown-writer.mjs`, `scripts/lib/path-utils.mjs`, `scripts/lib/vault-mirror/utils.mjs`.
...
**Output path:** `<vault-dir>/01-projects/_PORTFOLIO.md`. Frontmatter keys: `_generator: orchestrator-gitlab-portfolio@1`, `created` (ISO8601, set once), `updated` (ISO8601, refreshed each write), `repos` (count). Atomic write: content built in memory, committed via a single `writeFileSync`.
...
Configure `vault-integration.vault-dir` only with user-controlled paths under `~`. The same guard applies to the implicit Phase 2.7 portfolio snapshot at session-start. Reference: CWE-22 path traversal · `.trae/rules/security.md` SEC-014 adjacent guidance.
...
```markdown
---
_generator: orchestrator-gitlab-portfolio@1
...
|---|---|---|---|---|---|
| orchestrator | 9 | 2 | 1 | 0 | 2026-05-16 |
| my-saas-app          | 4 | 1 | 0 | 0 | 2026-05-15 |

## orchestrator

**gitlab:** `kanevry/orchestrator` · Open: 9 issues / 2 MRs · Critical: 1 · Next milestone: v3.7
...
- `_generator: orchestrator-gitlab-portfolio@1` MUST appear in every file written by this skill.
