...
VM_DIR=$(echo "$CONFIG" | jq -r '."vault-integration"."vault-dir" // empty')
: "${VM_DIR:=$VAULT_DIR}"
VAULT_DIR="$VM_DIR" bash ~/Projects/orchestrator/skills/daily/generate.sh
```
...
```bash
VAULT_DIR=~/Projects/vault bash ~/Projects/orchestrator/skills/daily/generate.sh
```

Or, if the user's cwd is already the vault:

```bash
cd ~/Projects/vault && bash ~/Projects/orchestrator/skills/daily/generate.sh
```
