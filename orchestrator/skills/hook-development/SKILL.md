...
{
  "type": "command",
  "command": "${ORCHESTRATOR_PLUGIN_ROOT}/hooks/validate.mjs",
  "timeout": 60
}
```
    ...
    "PreToolUse": [
      {
        "matcher": "Write|Edit",
        "hooks": [
          { "type": "command", "command": "${ORCHESTRATOR_PLUGIN_ROOT}/hooks/validate.mjs" }
        ]
      }
    ]
  }
...
### User `.orchestrator/settings.json` — direct format
...
| `$ORCHESTRATOR_PLUGIN_ROOT` | Plugin hooks | Plugin directory — **use this, never hardcode paths** |
...
{ "command": "${ORCHESTRATOR_PLUGIN_ROOT}/hooks/guard.mjs" }
...
echo '{"tool_name":"Write","tool_input":{"file_path":"/test"}}' | \
  ${ORCHESTRATOR_PLUGIN_ROOT}/hooks/guard.mjs
echo "Exit code: $?"
```
...
```bash
CONFIG_FILE="$CLAUDE_PROJECT_DIR/.orchestrator/config.json"
...
- Always `${ORCHESTRATOR_PLUGIN_ROOT}` for paths
...
`process.env.ORCHESTRATOR_PLUGIN_ROOT` directly. Use `resolvePluginRoot()` from
...
|-------|--------|-----------|
| 1 | `ORCHESTRATOR_PLUGIN_ROOT` env var | Returned immediately when set and is a directory |
| 2 | `CODEX_PLUGIN_ROOT` env var | Returned immediately when set and is a directory |
| 3 | Walk up from `import.meta.url` | Looks for `package.json` with `name: "orchestrator"` |
...
- [ ] `${ORCHESTRATOR_PLUGIN_ROOT}` for all paths
