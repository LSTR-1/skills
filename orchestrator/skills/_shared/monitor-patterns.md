...
while true; do
  ...
  jq -e 'all(.status=="success" or .status=="failed" or .status=="canceled" or .status=="skipped")' <<<"$s" >/dev/null 2>&1 && {
    sha=$(gh api repos/Kanevry/orchestrator/commits/main --jq '.sha' 2>/dev/null || echo "(mirror unreachable)")
...
`.trae/rules/loop-and-monitor.md` LM-003.
...
**timeout_ms.** `300000` (5 min — push usually completes in seconds; this is the Monitor wall-clock ceiling, **not** the `/loop` cadence band that LM-003 in `.trae/rules/loop-and-monitor.md` warns against)
...
prev=""
while true; do
  remote_sha=$(gh api repos/Kanevry/orchestrator/commits/main --jq '.sha' 2>/dev/null || echo "unreachable")
...
- **Unbounded `tail -f` for one-shot signals** — use Bash with
  `Task` dispatch mode and an `until` loop instead. See the Monitor tool
  docs.
...
- `.trae/rules/loop-and-monitor.md` — primitive routing (Monitor vs
  `/loop` vs Routines)
- `.orchestrator/loop.md` — bare-`/loop` body that delegates to Monitor where
  appropriate
