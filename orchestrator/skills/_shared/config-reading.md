...
> exists. Affected modules in this file: `scripts/lib/rule-loader.mjs`.

## Resolving the Plugin Root

`$ORCHESTRATOR_PLUGIN_ROOT` (Claude Code), `$ORCHESTRATOR_PLUGIN_ROOT` (Codex CLI), or `$ORCHESTRATOR_PLUGIN_ROOT` (Cursor IDE) may not be set (depends on how hooks/skills are loaded). Resolve the script path with this fallback chain:

1. If `$ORCHESTRATOR_PLUGIN_ROOT`, `$ORCHESTRATOR_PLUGIN_ROOT`, or `$ORCHESTRATOR_PLUGIN_ROOT` is set and non-empty, use it.
2. Otherwise, search for the plugin install location (includes Claude Code, Codex, and Cursor paths):
   ```bash
   PLUGIN_ROOT="${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}}"
   if [[ -z "$PLUGIN_ROOT" ]]; then
     # Check common install locations (Claude Code + Codex CLI + Cursor IDE)
     for candidate in \
       "$HOME/Projects/orchestrator" \
       "$HOME/.orchestrator/plugins/orchestrator" \
       "$HOME/.orchestrator/plugins/orchestrator" \
       "$HOME/plugins/orchestrator" \
       "$HOME/.orchestrator/plugins/orchestrator" \
...
Rule files at `.trae/rules/*.md` may carry an optional `globs:` YAML frontmatter field — an array of glob patterns relative to the repo root. The wave-executor calls `loadApplicableRules()` from `scripts/lib/rule-loader.mjs` before each wave to determine which rules apply.

### Glob-Scoped Rule Injection (#336)

**Frontmatter parsing.** At load time, `rule-loader.mjs` reads the YAML frontmatter block (delimited by `---`) from each `.md` file in `.trae/rules/`. The `globs:` key is expected to be a YAML sequence of glob-pattern strings (e.g. `- src/**/*.tsx`). Flow-style arrays (`globs: ["src/**", "tests/**"]`) are also accepted. Any parse error falls back to always-on (see Failure mode below).
...
const rules = loadApplicableRules({
  rulesDir: '.trae/rules',          // absolute path
