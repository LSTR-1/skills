...
```
Task({ subagent_type: "search", description: "Read session memory files",
  query: "Read session memory files for retrospective context. On Claude Code: read session-*.md in ~/.orchestrator/projects/<project>/memory/. On Codex CLI and Cursor IDE: skip this research — session memory is not available. Summarize: accomplishments, frustrations, scope changes, patterns." })

Task({ subagent_type: "search", description: "Analyze git log patterns",
  query: "Analyze git log (2 weeks). Identify: large commits (>20 files), hotfix patterns, reverts, frequency distribution." })

Task({ subagent_type: "search", description: "Analyze open issues for themes",
  query: "Read open issues from VCS. Categorize by type/area. Identify: recurring themes, repeated bugs, chronic carryover." })
```
