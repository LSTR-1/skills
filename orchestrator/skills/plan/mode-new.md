...
```
Task({ subagent_type: "search", description: "List available project archetypes",
  query: "Run: ls $BASELINE_PATH/templates/
...
Task({ subagent_type: "search", description: "Research market context",
  query: "Search online for market context related to the user's project idea.
  Report: target audiences, competitor landscape, positioning opportunities.
  Keep it to 5-10 bullet points with sources." })

Task({ subagent_type: "search", description: "Check ecosystem for conflicts",
  query: "Run: ls ~/Projects/
...
```
Task({ subagent_type: "search", description: "Read archetype config",
  query: "Read $BASELINE_PATH/templates/{archetype}/ — list all files, read key configs.
  Report: default tech stack, available options, required decisions." })

Task({ subagent_type: "search", description: "Research tech stack best practices",
  query: "Search online for best practices with the chosen tech stack ({archetype} + {key techs}).
  Report: recommended patterns, common pitfalls, performance tips. 5-10 bullet points." })

Task({ subagent_type: "search", description: "Check ecosystem for shared patterns",
  query: "Explore repos in ~/Projects/ that use similar tech stacks.
...
```
Task({ subagent_type: "search", description: "Analyze archetype failure modes",
  query: "Search online for common failure modes and risks for {archetype} projects.
  Report: top 5 risks with probability/impact and standard mitigations." })

Task({ subagent_type: "search", description: "Check ecosystem dependency compatibility",
  query: "For each ecosystem repo mentioned by the user as a dependency,
...
Task({ subagent_type: "search", description: "Research success benchmarks",
  query: "Search online for comparable products/features to the user's project idea.
