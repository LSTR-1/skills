...
> exists. Affected modules in this skill: `scripts/lib/slopcheck.mjs`.
...
Before each Q&A wave, dispatch 2-3 `Agent()` tool calls in a single message (parallel execution) with `subagent_type: "search"`:
...
```
Task({ subagent_type: "search", description: "Research market context for [topic]",
  query: "Search online for [topic]. Report findings with pros/cons for each option." })
Task({ subagent_type: "search", description: "Analyze baseline templates",
  query: "Read projects-baseline at $BASELINE_PATH/templates/. List available archetypes, their README descriptions, and key features." })
Task({ subagent_type: "search", description: "Analyze repo context",
  query: "Explore current repo for [relevant patterns]. Report affected files and existing conventions." })
```

Wait for ALL agents to complete before proceeding. Use synchronous `Task` dispatch (wait for completion) for all agents.
