...
> exists. Affected modules in this skill: `scripts/lib/agent-frontmatter.mjs`, `scripts/lib/agent-output-schema.mjs`, `scripts/lib/io.mjs`, `scripts/lib/path-utils.mjs`, `scripts/lib/persona-panel/catalog-loader.mjs`, `scripts/lib/persona-panel/consolidator.mjs`, `scripts/lib/persona-panel/persona-runner.mjs`.
...
The catalog lives in `.orchestrator/personas/*.md` — per-repo, never plugin-central. This is
...
**(a) `.orchestrator/personas/` directory missing:**
```
Error (exit 2): .orchestrator/personas/ directory not found in this repo.
Create persona files there to use /persona-panel.
See templates/personas/ for starter templates (issue #458).
```

**(b) `.orchestrator/personas/` present but empty (no `.md` files):**
```
Error (exit 2): .orchestrator/personas/ exists but contains no persona files (*.md).
...
```
Error (exit 1): Persona "<name>" not found in .orchestrator/personas/.
Available personas: <list of names from catalog>.
```

**(d) Malformed YAML frontmatter in a catalog file:**
```
Error (exit 1): Malformed YAML in .orchestrator/personas/<filename>.md at line <N>: <error>.
...
```
Catalog: [N] personas loaded from .orchestrator/personas/. Tier breakdown: domain-expert [N], buyer-persona [N], compliance [N], custom [N].
```
...
Task({
  ...
  query: <buildPersonaPrompt(persona, $TARGET)>,
...
**Dispatch mode:** synchronous `Task` dispatch for all agents. Do not proceed to Phase 4 until ALL agents
complete.
...
- **NEVER** use fire-and-forget dispatch (NOT supported) for persona agents — lose coordination ability.
