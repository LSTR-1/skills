...
> Catalog location: `.orchestrator/personas/*.md` (per-repo, never plugin-central).

## File Layout

Each persona is a single Markdown file with YAML frontmatter followed by a structured body.
Files live at `.orchestrator/personas/<name>.md` where `<name>` matches the frontmatter `name:` field.
...
| `model` | string | Model alias (`inherit`). Validated at load time. **Port note:** the original plugin pinned a Claude model ID here; this port delegates model selection to the TRAE runtime and accepts only `inherit`. |
