...
Note: `.orchestrator/` is NOT gitignored — `bootstrap.lock` must be committed. Only the platform state dirs (`.orchestrator/`, `.orchestrator/`, `.orchestrator/`) are excluded.
...
```bash
cd "$REPO_ROOT"
BOOTSTRAP_FILES=(CLAUDE.md AGENTS.md .gitignore README.md .orchestrator/bootstrap.lock .trae/rules/parallel-sessions.md)
