...
description: >
  Use this skill when scaffolding the minimum repository structure required by orchestrator.
...
> exists. Affected modules in this skill: `scripts/lib/config-schema.mjs`, `scripts/lib/ecosystem-wizard.mjs`, `scripts/lib/fetch-baseline.mjs`, `scripts/lib/owner-interview.mjs`, `scripts/lib/package-manager.mjs`, `scripts/lib/product-repo-detect.mjs`, `scripts/lib/rules-sync.mjs`.

## Overview

This skill runs when the Bootstrap Gate is closed (missing CLAUDE.md, Session Config, or `.orchestrator/bootstrap.lock`) or when the user invokes `/bootstrap` directly. It scaffolds the minimum structure required by all orchestrator skills, commits it, and writes the lock file that opens the gate for all future invocations.
...
**Purpose:** Vendor canonical rules from the plugin's `rules/` library (`rules/always-on/*.md`, and in the future `rules/opt-in-stack/*.md` and `rules/opt-in-domain/*.md`) into the consumer repo's `.trae/rules/`. Plugin-sourced files (identified by a `<!-- source: orchestrator plugin … -->` header) are overwritten on re-run; files without that header are preserved as local overrides. See `rules/_index.md` for the canonical manifest and `scripts/lib/rules-sync.mjs` for the implementation.
...
2. **Invoke `scripts/lib/rules-sync.mjs`.** Run the CLI entrypoint from the consumer repo:
   ...
   The script reads `rules/_index.md` from the plugin, iterates `always-on/` sources, and writes each file into `.trae/rules/` under the target repo. Stdout is a JSON object with `written[]`, `skipped[]`, `preserved[]`, and `errors[]`. Exit 1 on any error, 0 otherwise.
...
3. **Interpret the output.** Report a human summary:
   ...
   - `preserved`: existing `.trae/rules/*.md` files that do NOT carry the plugin source header — left untouched as local overrides.
...
**Local overrides.** Any `.trae/rules/<name>.md` without the plugin source header is considered local and never overwritten. To replace a local override with the canonical version, delete it before re-running.
...
> Closes orchestrator issues #175 (D2 owner interview) + #173 (C4 hardware-sharing consent).

**WHEN:** Runs after Phase 3.4 when `~/.config/orchestrator/owner.yaml` is absent AND `--no-interview` was NOT passed. Skipped entirely on `--upgrade`, `--retroactive`, `--sync-rules`, and `--ecosystem-health` flows.
...
**WHERE:** Written to `~/.config/orchestrator/owner.yaml` (user-global, never committed).
...
> Closes orchestrator issue #110.

After the tier template completes scaffolding (Phase 3), the Standard and Deep templates run an optional rules-fetch step that pulls canonical `.trae/rules/*.md` (and optionally `.orchestrator/agents/*.md`) directly from the baseline GitLab project. The step is opt-in and only fires when:
...
3. Writes `.orchestrator/.baseline-fetch.lock` (via an inline `node --input-type=module -e`) recording what was fetched
4. Populates `.orchestrator/.baseline-cache/` for offline fallback on subsequent invocations
...
**Local edits:** Re-running bootstrap with `baseline-ref` set will overwrite `.trae/rules/*.md` (rules are canonical). Repo-specific extensions belong in `.trae/rules/local/*.md` (not fetched, not overwritten).
...
### `.orchestrator/.baseline-fetch.lock` Schema

The lock file is committed to git and records what was fetched.

```yaml
# .orchestrator/.baseline-fetch.lock
version: 1
project_id: 52
baseline_ref: main
...
After all template files are written and committed, write `.orchestrator/bootstrap.lock` (and, if the rules-fetch bridge ran, also `.orchestrator/.baseline-fetch.lock` — see Phase 3.6):
...
Read `plugin-version` from the orchestrator plugin's `package.json` (`$PLUGIN_ROOT/package.json`, field `version`). This enables the freshness probe (#290) to detect plugin upgrades that need re-bootstrap. Example:
