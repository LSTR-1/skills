...
```bash
if [[ -n "${ORCHESTRATOR_PLUGIN_ROOT:-}" ]]; then
  PLATFORM="trae"
else
  # Fallback: inspect running process name or default to trae
  PLATFORM="trae"
fi
```

> **Port note:** The original plugin detected `claude` / `codex` / `cursor` via three distinct
> environment variables. This port targets TRAE IDE SOLO Mode and collapses the detection to a
> single `ORCHESTRATOR_PLUGIN_ROOT` variable. The `PLATFORM` value is `trae`.
