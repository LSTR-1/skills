...
`$REPO_ROOT/.trae/rules/parallel-sessions.md` (idempotent: missing→create, identical→skip,
...
`scripts/lib/fetch-baseline.mjs` exists. Fetches `.trae/rules/*.md` from the baseline GitLab
project (default project 52) and writes `.orchestrator/.baseline-fetch.lock`. Does NOT abort on failure.
...
plugin-version: <orchestrator plugin version — read from $PLUGIN_ROOT/package.json .version field>
...
Scaffold `.orchestrator/STATE.md` from `skills/bootstrap/STATE.md.template` (idempotent: skip if
already exists). On Codex CLI / Cursor IDE, substitute `.orchestrator/` or `.orchestrator/` for `.orchestrator/`.
See shared partial for full shell command. Issue #184.

<!-- @include _shared-template.md#agents-scaffold -->
## Step 6.7: .orchestrator/agents/ Scaffold (#189)
...
`project-quality-gate`) into `$REPO_ROOT/.orchestrator/agents/`. Idempotent: skip existing files.
...
BOOTSTRAP_FILES=(
  ...
  .editorconfig src/ tests/ .orchestrator/
)
...
Bootstrap (standard, <archetype>) complete. Created:
  ...
  .trae/rules/parallel-sessions.md   — vendored PSA rule (issue #155)
  .orchestrator/STATE.md                      — idle placeholder (issue #184)
