...
Write the vendored rule from `$PLUGIN_ROOT/templates/_shared/rules/parallel-sessions.md` to `$REPO_ROOT/.trae/rules/parallel-sessions.md`.
...
Shell:
```bash
mkdir -p "$REPO_ROOT/.trae/rules"
cp "$PLUGIN_ROOT/templates/_shared/rules/parallel-sessions.md" "$REPO_ROOT/.trae/rules/parallel-sessions.md"
```
...
## #agents-scaffold — Step: .orchestrator/agents/ Scaffold (#189)

Copy the opinionated agent templates into the consumer repo:

```bash
mkdir -p "$REPO_ROOT/.orchestrator/agents"
cp "$PLUGIN_ROOT/skills/bootstrap/templates/agents/"*.md "$REPO_ROOT/.orchestrator/agents/"
```
...
**Idempotency:** Existing files under `.orchestrator/agents/` are not overwritten — skip any file that already exists.
...
- The orchestrator plugin includes `scripts/lib/fetch-baseline.mjs`
- A GitLab host is resolvable (`gitlab-host` Session Config key or `GITLAB_HOST` env)

When triggered, this step pulls the canonical `.trae/rules/*.md` and (optionally) `.orchestrator/agents/*.md` files directly from the baseline GitLab project (project 52 by default) into the new repo, then writes `.orchestrator/.baseline-fetch.lock` recording the fetch.
...
.trae/rules/security-web.md
.trae/rules/security-compliance.md
.trae/rules/testing.md
.trae/rules/test-quality.md
.trae/rules/frontend.md
.trae/rules/backend.md
.trae/rules/backend-data.md
.trae/rules/infrastructure.md
.trae/rules/swift.md
.trae/rules/mvp-scope.md
.trae/rules/cli-design.md
.trae/rules/parallel-sessions.md
.trae/rules/ai-agent.md
.trae/rules/claude-code-usage.md
MANIFEST
  ...
  if [[ -s "$SUCCESS_LOG" ]]; then
    ...
    LOCK_FILE="$REPO_ROOT/.orchestrator/.baseline-fetch.lock"
    ...
    echo "Wrote .orchestrator/.baseline-fetch.lock ($(wc -l < "$SUCCESS_LOG" | tr -d ' ') files)"
  else
...
**Idempotency:** Re-running bootstrap on an existing repo will overwrite `.trae/rules/*.md` files. Local edits to baseline rules in a repo will be lost on re-fetch — this is intentional (rules are canonical). Repo-specific extensions belong in `.trae/rules/local/*.md` (not fetched).
...
Scaffold a placeholder `.orchestrator/STATE.md` using the template at `skills/bootstrap/STATE.md.template`. The placeholder records `status: idle` — sessions overwrite it at Pre-Wave 1b.

**Idempotency:** Skip if `.orchestrator/STATE.md` already exists.

```bash
STATE_FILE="$REPO_ROOT/.orchestrator/STATE.md"
...
On Codex CLI / Cursor IDE, substitute `.orchestrator/` or `.orchestrator/` for `.orchestrator/` per the platform state-directory convention.
