...
```yaml
# .gitlab-ci.yml — generated by orchestrator bootstrap (deep)
...
```yaml
# .github/workflows/ci.yml — generated by orchestrator bootstrap (deep)
...
```
# CODEOWNERS — generated by orchestrator bootstrap (deep)
...
Schema: https://github.com/Kanevry/orchestrator
...
If S99 ran successfully, `.trae/rules/*.md` and `.orchestrator/.baseline-fetch.lock` are already written
to `$REPO_ROOT`. These files are included in the Deep commit at Step D8 via the `.orchestrator/` entry
...
## Step D6.5: .orchestrator/agents/ Scaffold (#189)
