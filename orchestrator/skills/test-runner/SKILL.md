...
> exists. Affected modules in this skill: `scripts/lib/autopilot/telemetry.mjs`, `scripts/lib/discovery/helpers.mjs`, `scripts/lib/playwright-driver/runner.mjs`, `scripts/lib/test-runner/artifact-paths.mjs`, `scripts/lib/test-runner/check-playwright-mcp-canary.mjs`, `scripts/lib/test-runner/fingerprint.mjs`, `scripts/lib/test-runner/issue-reconcile.mjs`.
...
Task({
  ...
  query: `<scope: ${RUN_DIR}, rubric: skills/test-runner/rubric-v1.md, output: ${RUN_DIR}/findings.jsonl>`,
  subagent_type: "general_purpose_task",
})
```

synchronous `Task` dispatch (wait for completion) is mandatory — Phase 4 depends on `findings.jsonl` being fully written before reconciliation begins.
...
- **ALWAYS** set synchronous `Task` dispatch (wait for completion) for the `ux-evaluator` agent dispatch — Phase 4 depends on its output.
