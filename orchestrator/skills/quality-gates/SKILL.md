...
> exists. Affected modules in this skill: `scripts/lib/quality-gates-cache.mjs`, `scripts/lib/quality-gates-policy.mjs`.
...
node "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/run-quality-gate.mjs" --variant baseline --config "$CONFIG"

# Incremental (wave-executor)
node "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/run-quality-gate.mjs" --variant incremental --config "$CONFIG" --files changed-file1.ts,changed-file2.ts

# Full Gate (session-end)
node "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/run-quality-gate.mjs" --variant full-gate --config "$CONFIG" --session-start-ref "$SESSION_START_REF"

# Per-File (session-reviewer)
node "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/run-quality-gate.mjs" --variant per-file --config "$CONFIG" --files specific-file.ts
```
