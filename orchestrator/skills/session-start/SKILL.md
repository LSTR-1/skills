...
> exists. Affected modules in this skill: `scripts/lib/autopilot/worktree-pipeline.mjs`, `scripts/lib/bootstrap-lock-freshness.mjs`, `scripts/lib/ci-status-banner.mjs`, `scripts/lib/coordinator-snapshot.mjs`, `scripts/lib/gitlab-portfolio/cli.mjs`, `scripts/lib/historical-guard.mjs`, `scripts/lib/memory-banner.mjs`, `scripts/lib/memory-paths.mjs`, `scripts/lib/peer-cards/staleness-banner.mjs`, `scripts/lib/peer-discovery.mjs`, `scripts/lib/qg-command-drift-banner.mjs`, `scripts/lib/recommendations-v0.mjs`, `scripts/lib/session-lock.mjs`, `scripts/lib/state-md-peer-guard.mjs`, `scripts/lib/state-md.mjs`, `scripts/lib/vault-staleness-banner.mjs`.
...
> Where `<state-dir>` is `.orchestrator/` under Claude Code or `.orchestrator/` under Codex CLI. See `skills/_shared/platform-tools.md` for details.
...
1. Ensure '.orchestrator/metrics/' directory exists in the project root (create if missing). For backward compatibility with pre-v2.0 sessions, also check the platform's legacy metrics directory (`<state-dir>/metrics/` where `<state-dir>` is `.orchestrator/`, `.orchestrator/`, or `.orchestrator/` per platform).
...
4. **Plugin freshness**: Determine the orchestrator plugin directory (navigate up from this skill's base directory to the plugin root). Run `git -C <plugin-dir> log -1 --format="%ci"` to get the last commit date. If older than `plugin-freshness-days` (default: 30) days, flag a warning in the Session Overview: `"⚠ Session Orchestrator plugin last updated [N] days ago — consider pulling the latest version."` Non-blocking — present in overview, don't halt.
   ...
   - **Drift detected** (`{ severity: 'warn', message: ... }`): render `result.message`. The message has the shape `"⚠ Session Config drift (*-command keys): <details>. Verify the overrides are intentional. See .trae/rules/quality-gates-autofix.md § Session Config Command Injection for the RCE-equivalent trust-model."`
   - **No drift**: silent (no banner).

   The banner is non-blocking — display in the Session Overview, do not halt the session. Cross-reference: `.trae/rules/quality-gates-autofix.md` § Session Config Command Injection — the banner exists because `*-command` keys are RCE-equivalent under the VCS trust-anchor model.
   ...
   Cross-reference: `.trae/rules/owner-persona.md` (host-wide `owner.yaml` operator identity) and `skills/vault-sync/SKILL.md` (`type: peer-card` value in the vault-frontmatter enum). Peer cards complement `owner.yaml` with per-repo behavioural identity for the operator (USER.md) and agent (AGENT.md).
...
> **Platform Note:** Session memory files at `~/.orchestrator/projects/` are a Claude Code feature. On Codex CLI and Cursor IDE, skip this phase — per-project memory persistence is not available on those platforms.

Surface context from previous sessions:

1. Look for session memory files at `~/.orchestrator/projects/<project>/memory/session-*.md`
...
- Run token efficiency check: `bash "${ORCHESTRATOR_PLUGIN_ROOT:-$PLUGIN_ROOT}/scripts/token-audit.sh"` and include findings in Session Overview. Flag any HIGH/WARN items as recommended housekeeping tasks.
...
- **ALWAYS use synchronous `Task` dispatch (wait for completion)** for parallel subagent work — wait for completion
