...
> **Platform Note:** Project agents live in `<state-dir>/agents/` where `<state-dir>` is `.orchestrator/` (Claude Code), `.orchestrator/` (Codex CLI), or `.orchestrator/` (Cursor IDE). On Cursor IDE, parallel agent dispatch is not available — present wave tasks as a sequential execution list instead. See `skills/_shared/platform-tools.md`.
...
> exists. Affected modules in this skill: `scripts/lib/autopilot/worktree-pipeline.mjs`, `scripts/lib/mission-status-schema.mjs`, `scripts/lib/state-md.mjs`.
...
1. **Scan for project-level agents**: Glob `<state-dir>/agents/*.md` (`.orchestrator/agents/*.md` for Claude Code, `.orchestrator/agents/*.md` for Codex CLI, `.orchestrator/agents/*.md` for Cursor IDE)
...
3. **Build Agent Registry** (resolution priority):
   ...
   - **Priority 2**: Plugin agents (`orchestrator:code-implementer`, `orchestrator:test-writer`, `orchestrator:ui-developer`, `orchestrator:db-specialist`, `orchestrator:security-reviewer`)
...
4. **Match tasks to agents**: For each task from Step 1:
   ...
   - **Docs-role fast path (high-priority — runs before keyword matching):** If the task's role is classified as `Docs` (per Step 1.8) AND `docs-orchestrator.enabled: true` in Session Config → resolve `subagent_type: "general_purpose_task"`. The `docs-writer` project agent is discovered at `<state-dir>/agents/docs-writer.md` during the Priority 1 scan above. No colon prefix — it is a project agent, not a plugin agent. If `agent-mapping.docs` is set, use that name instead of `"docs-writer"`.
   - Else, match task description against agent descriptions using the content-based routing table below. Match any keyword from the pattern column (case-insensitive) against the task title and description. Use the first matching row; rows are checked top to bottom.
     ...
     |---|---|
     | `migration`, `schema`, `RLS`, `index`, `query`, `ORM`, `supabase`, `postgres`, `database`, `db` | `orchestrator:db-specialist` |
     | `component`, `tsx`, `css`, `tailwind`, `page`, `layout`, `a11y`, `wcag`, `responsive`, `UI`, `frontend`, `style` | `orchestrator:ui-developer` |
     | `security`, `auth`, `csrf`, `csp`, `injection`, `XSS`, `sanitize`, `OWASP`, `vulnerability`, `pen test` | `orchestrator:security-reviewer` |
     | `test`, `coverage`, `vitest`, `jest`, `playwright`, `spec`, `fixture`, `assertion` | `orchestrator:test-writer` |
     | (none of the above match) | `orchestrator:code-implementer` |
...
- Distributional claims MUST follow `.trae/rules/parallel-sessions.md` § PSA-006 — quote the executed grep pattern + file scope + count. Coordinators REJECT Discovery outputs that assert "N of M" or "100% of X" without a quoted grep transcript (deep-1647 W1-D3 incident class).
...
- Example: "database-architect (project) → DB tasks, orchestrator:code-implementer (plugin) → API tasks"
